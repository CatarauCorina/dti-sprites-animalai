...
class DTISprites(nn.Module):
    ...
    def predict(self, x):
        ...
        if self.inject_noise and self.training:
            ...
            tsf_masks_with_noise = tsf_masks + tsf_noise * (2 * self.inject_noise * noise - self.inject_noise)
            tsf_masks_clamped = self.clamp_func(tsf_masks_with_noise)
            tsf_masks = tsf_masks_clamped
