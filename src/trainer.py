...
class Trainer:
    ...
    def __init__(self, config_path, run_dir):
        ...
        # Visdom
        if cfg['training']['is_hyperion']:
            os.environ['https_proxy'] = "http://hpc-proxy00.city.ac.uk:3128"

        logger_name = f'dti-sprites-{self.dataset_name}'
        logger = wandb.init(project='dti-sprites', name=logger_name)
        self.visualizer = logger
    ...
    def update_visualizer_images(self, images, title, nrow):
        ...
        images = wandb.Image(images.clamp(0, 1))
    ...
    def update_visualizer_metrics(self, cur_iter, train):
        ...
        if train:
            if self.n_prototypes > 1:
                test_table = wandb.Table(columns=['metrics'])
                # Cluster proportions
                N = self.n_clusters if self.n_clusters <= 40 else 2 * self.n_prototypes
                for i in range(N):
                    test_table.add_data(metrics[f'prop_clus{i}'].avg)
                ...
                wandb.log({"train_cluster_proportions": wandb.plot.bar(test_table, "label", "value",
                                                                       title="train_cluster_proportions")})

        # else:
        #     names = list(filter(lambda name: 'cls' not in name, self.val_scores.names))
        #     y_old, x_old = [[self.val_scores[n] for n in names]], [[cur_iter] * len(names)]
        #     x_values = [[cur_iter] * len(names)]
        #     y_values = [[self.val_scores[n] for n in names]]
        #     data = [[x, y] for (x, y) in zip(x_values, y_values)]
        #     table = wandb.Table(data=data, columns=["x", "y"])
        #     wandb.log({"global_scores": wandb.plot.line(table, "x", "y",
        #                                                     title="global_scores")})
        ...
        #         N = self.n_classes
        #         y_values = [[self.val_scores[f'{name}_cls{i}'] for i in range(N)]]
        #         x_values = [[cur_iter] * N]
        #         data = [[x, y] for (x, y) in zip(x_values, y_values)]
        #         table = wandb.Table(data=data, columns=["x", "y"])
        #         wandb.log({f'{name}_by_cls': wandb.plot.line(table, "x", "y",
        #                                                         title=f'{name}_by_cls')})


                # self.visualizer.line(y, x, win=f'{name}_by_cls', update='append', opts=dict(title=f'{name}_by_cls',
                #                                                                             legend=[f'cls{i}' for i in
                #                                                                                     range(N)],
                #                                                                             width=VIZ_WIDTH,
                #                                                                             heigh=VIZ_HEIGHT))
